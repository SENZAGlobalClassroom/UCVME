...
function generateCV()
{
    ...
    // combining name and email in the QR code data
    const cvData = `Name: ${name}\nEmail: ${email}\n`;
    
    // clear existing QR code
    const qrcodeElement = document.getElementById('qrcode'); // get html with id qrcode
    qrcodeElement.innerHTML = ''; // sets inner html to an empty string to clear existing content
    
    // Correct instantiation of qrcode
    const qrcode = new QRCode(qrcodeElement); // instantiate new QR code passing qrcode
    qrcode.makeCode(cvData); // call makeCode method with cvData generating the QR
}